...
Button::Button(float x, float y, color_t color) {
    ...
    for( int i =0 ; i<360 ; i++){
        ...
        for(int t=0;t<3;t++){
            vertex_color_data[j+3*t+0] = color.r;
            vertex_color_data[j+3*t+1] = color.g;
            vertex_color_data[j+3*t+2] = color.b;
            temp = color;
        }
    }
...
void Button::lighting(int light, float posx, float posy)
{
    if(light==0)
    {
        for( int i =0 ; i<360 ; i++){
            int j = i*9;
            for(int t=0;t<3;t++){
                vertex_color_data[j+3*t+0] = temp.r;
                vertex_color_data[j+3*t+1] = temp.g;
                vertex_color_data[j+3*t+2] = temp.b;
            }
        }
